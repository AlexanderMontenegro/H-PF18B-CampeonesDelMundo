...
const Orden = ({
  ...
  useEffect(() => {
    ...
    initMercadoPago("TEST-5af066e7-e8b7-4bdc-b45a-f61db3669c4e", {
      locale: "es-AR",
    });
  }, []);
  ...
  const notificacionCompra = (carrito, carritoTotal) => {
    const items = carrito
      .map((item) => item.quantity + " " + item.tipo)
      .join(", ");
    addToNotificaciones(
      `FELICIDADES por su compra en: ${items}. Con un importe de: ${carritoTotal}`
    );
  };
          ...
          <div className="pago__total">
            ...
            <div className="suma__total">
              <p className="btn__or_txt">
                Total pagar <span className="btn__or_2">${carritoTotal()}</span>
              </p>
            </div>
